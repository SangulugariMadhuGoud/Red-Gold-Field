  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/public/site-visit`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to submit visit request");
      }
        ...
        <div className="relative z-10 container mx-auto px-6 max-w-3xl text-center mx-auto pt-10">
          <FadeUp>
            ...
            <p className="text-muted-foreground text-xl leading-relaxed">
              Visiting the land is essential before making any decision. We
              arrange dedicated site visits with guidance from our team.
            </p>
          </FadeUp>
        </div>
      </div>
            ...
            {[
              {
                ...
                desc: "Physically tour the land, see plot boundaries, internal roads, and plantation areas.",
              },
              {
                icon: Clock,
                title: "Team Guidance",
                desc: "Our team explains every aspect — no rushed presentations, just honest conversation.",
              },
              {
                icon: CheckCircle2,
                title: "Document Review",
                desc: "All project documents reviewed on-site or shared digitally beforehand.",
              },
            ].map((s, i) => (
            ...
            {submitted ? (
              <div className="bg-gradient-card border border-gold/20 rounded-3xl p-14 text-center">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  ...
                  date and provide all necessary details.
                </p>
              </div>
            ) : (
              <form
                onSubmit={handleSubmit}
                className="bg-gradient-card border border-border/40 rounded-3xl p-8 space-y-5"
              >
                ...
                <p className="text-xs text-muted-foreground/60 text-center">
                  No commitment required. Our team will contact you to confirm
                  details.
                </p>
              </form>
            )}
